...
const buyProductpost = async (
  ...
  try {
    // Kiểm tra và gán giá trị mặc định nếu tham số là undefined
    ten = ten || null;
    diachi = diachi || null;
    ghichu = ghichu || null;
    ...
    if (results.length > 0) {
      const [results1, fields1] = await connection.execute(
        `INSERT INTO donhang(madonhang, makhachhang, ngaydonhang, trangthai, ten, diachi, sodienthoai, ghichu, phuongthucthanhtoan) 
         VALUES (?, ?, ?, 'ChuaGiao', ?, ?, ?, ?, ?)`,
        [
          newIdDonhang,
      ...
      const [results2, fields2] = await connection.execute(
        `INSERT INTO chitietdonhang(madonhang, masp, soluong, thanhtien, trangthai, SIZE) 
         VALUES (?, ?, ?, ?, 'ChuaGiao', ?)`,
      ...
      const [results1, fields1] = await connection.execute(
        `INSERT INTO donhang(madonhang, makhachhang, ten, diachi, sodienthoai, ghichu, ngaydonhang, trangthai, phuongthucthanhtoan) 
         VALUES (?, ?, ?, ?, ?, ?, ?, 'ChuaGiao', ?)`,
        [
          newIdDonhang,
          newId,
    ...
    console.error("Error in buyProductpost:", error);
    throw error;
  }
};
