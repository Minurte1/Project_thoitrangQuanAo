...
export default function ShopProductCard({ product, callback }) {
  ...
  const handleUpdateProducts = async () => {
    ...
    if (SoLuongShoe > 200) {
      toast.error("Số lượng quần áoquá lớn rồi :<");
      return;
    }
  ...
  const XacNhanTaoHangGiay = async () => {
    try {
      ...
      toast.success("Thêm hãng quần áothành công");
    } catch (error) {
      console.error(error);
      toast.error("Thêm hãng quần áokhông thành công");
    }
  };
  ...
  const XacNhanTaoLoaiGiay = async () => {
    try {
      ...
      toast.success("Thêm loại quần áothành công");
    } catch (error) {
      console.error(error);
      toast.error("Thêm loại quần áokhông thành công");
    }
  };
  ...
  const giamgia = formatCurrency(
    (product.GIA - (product.GIA * product.giamgia) / 100).toFixed(0)
  );
  ...
  const handleDeleteProducts = () => {
    ...
    axios
      .delete(url)
      .then((response) => {
        toast.success("Xóa sản phẩm quần áothành công!!");
      ...
      .catch((error) => {
        ...
        toast.error("Xóa sản phẩm quần áokhông thành công!!");
        // Xử lý lỗi nếu có
      });
  };
        ...
        <div className="container-modalRegister">
          <div className="modalRegister">
                  ...
                  if (hang.MAHANG === product.MAHANG) {
                    return (
                      <option
                        ...
                        value={HanggiayShoe}
                        onChange={(event) =>
                          setHanggiayShoe(event.target.value)
                        }
                      >
                        {hang.TENHANG}
                      </option>
              ...
              >
                {DataLoaiBackend.map((loai) => {
                  if (loai.MALOAI === product.MALOAI) {
                    return (
                      <option selected key={loai.MALOAI} value={loai.MALOAI}>
                        {loai.name}
                      </option>
                    );
                  } else {
            ...
            <div className="modalRegister-sdt margin5px">
              ...
              />
              {selectedImage && (
                <label> Chosen file: {selectedImage.name}</label>
              )}
            </div>
            <div className="modalRegister-sdt margin5px">
              <button
