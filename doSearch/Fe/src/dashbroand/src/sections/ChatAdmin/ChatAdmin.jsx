  ...
  const SendMessNe = async () => {
    ...
    try {
      ...
      if (IdCoversation) {
        ...
        const responseMess = await axios.post(`${ENDPOINT}/api/getMessages`, {
  ...
  const handleUserIb = async (user) => {
    ...
    try {
      const response = await axios.get(
        `http://localhost:3001/api/getUserByUsername/${user.taikhoan}`
      );
    ...
    try {
      const response = await axios.get(
        `http://localhost:3001/api/getUserByUsername/${usernameAdmin}`
      );
      const { userId } = response.data;
      ...
      <div className="container-chat-admin">
        <div className="container-chat-admin-headerListUser">
          {DataAllUserBackend.map((users) => (
            <div
              className={`headerListUser ${
                selectedUser === users.taikhoan ? "selected" : ""
              }`}
              onClick={() => handleUserIb(users)}
            >
              <img
                src={
                  users.avatar
                    ? `http://localhost:3003/images/${users.avatar}`
                    : `http://localhost:3003/images/avatatrang.jpg`
                }
        ...
        <div className="chat-container" ref={chatContainerRef}>
          {TinNhan.map((message) => (
            <div
              key={message._id}
              className={`message ${
                message.username !== "admin" ? "message-left" : "message-right"
              }`}
            >
              ...
              {message.username !== "admin" && (
                <div className="container-messCha">
                  <img
                    ...
                    src={
                      AvataKH
                        ? `http://localhost:3003/images/${AvataKH}`
                        : `${avat}`
                    }
                    alt="User Avatar"
                  />
