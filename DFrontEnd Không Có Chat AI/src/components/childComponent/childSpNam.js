...
export const ChildSPNam = ({ shoes, hang }) => {
  ...
  const [selectHang, setSelectHang] = useState("");
  ...
  console.log("select Hang => ", selectHang);
  ...
  return (
    <div className="shoe-list">
      ...
      <div className="dropdown-container">
        <label className="label-price" htmlFor="brandFilter">
          Chọn hãng:
        </label>
        <select
          id="brandFilter"
          value={selectHang}
          onChange={(e) => setSelectHang(e.target.value)}
          className="select-price"
        >
          <option value="Tất cả">Tất cả</option>
          {hang.map((hang, index) => (
            <option key={index} value={hang.MAHANG}>
              {hang.TENHANG}
            </option>
          ))}
        </select>
