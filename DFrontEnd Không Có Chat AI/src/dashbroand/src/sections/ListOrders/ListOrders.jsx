...
const ListOrders = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await axiosWithCredentials.get(
        "http://localhost:3003/api/v1/donhangchuagiao"
      );
      const sortedOrders = response.data.DT.sort(
        (a, b) => new Date(b.ngaydonhang) - new Date(a.ngaydonhang)
      );
  ...
  const handleXacNhanGiaoHang = async (madonhang) => {
    ...
    try {
      const response = await axios.put(
        `http://localhost:3003/api/v1/donhang/update/${madonhang}`
      );
  ...
  const handleHuyDon = async (madonhang) => {
    ...
    try {
      const response = await axios.put(
        `http://localhost:3003/api/v1/donhanghuy/update/${madonhang}`
      );
  ...
  const formatDate = (dateString) => {
    ...
    const formattedDate = new Intl.DateTimeFormat("vi-VN", optionsDate).format(
      date
    );
  ...
  const filteredOrder = ListOdersChuaGiao.find(
    (order) => order.madonhang === MaDonHang
  );
  return (
    <>
      <h2>Đơn Hàng</h2>
      <div className="wrap">
        ...
        <button
          ...
          onClick={handleMoveRouteChiTietHoaDon}
        >
          Chi Tiết Hóa Đơn
        </button>
      </div>
            ...
            <tr key={index}>
              ...
              <td>
                <button
                  ...
                  onClick={() => handleXacNhanGiaoHang(order.madonhang)}
                >
                  Xác Nhận
                </button>
              </td>
              <td>
                <button
                  ...
                  onClick={() => handleHuyDon(order.madonhang)}
                >
                  Hủy Đơn
                </button>
              </td>
              <td>
                <button
                  className="btn btn-info"
                  onClick={() => handleChiTiet(order.madonhang)}
                >
                  Chi Tiết
                </button>
              </td>
        ...
        <table className="table">
          ...
          <tbody>
            <tr>
              ...
              <td>
                {filteredOrder.giamgia != null
                  ? `${filteredOrder.giamgia}%`
                  : "không giảm giá"}
              </td>
              ...
              <td>
                <img
                  src={`http://localhost:3003/images/${filteredOrder.description}`}
                  alt={filteredOrder.description}
                  width="50"
                />
              </td>{" "}
              <td>{filteredOrder.SIZE}</td>
            </tr>
          </tbody>
        </table>
      )}
